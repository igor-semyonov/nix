  ...
  in {
    options.${ns}.u2f = {
      ...
      userKeys = lib.mkOption {
        description = "Attrset of users to list of keys for each user. The keys are generated using `pamu2fcfg -o pam://<sharedid>`";
        type = lib.types.attrsWith {
          elemType = lib.types.listOf lib.types.singleLineStr;
    ...
    config = lib.mkIf cfg.enable {
      ...
      environment.etc.${cfg.authfile}.text =
        lib.strings.join "\n"
        (
          lib.mapAttrsToList (
            user: keys: lib.strings.join ":" ([user] ++ keys)
          )
          cfg.userKeys
        );
    };
